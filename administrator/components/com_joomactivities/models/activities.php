...
JoomActivitiesHelper::loadPluginLanguages();
 ...
 *
 */
class JoomActivitiesModelActivities extends JModelList
{
    /**
    ...
    {
        $params = JComponentHelper::getParams('com_joomactivities');
    ...
    public function getItems()
    {
        ...
        if (JDEBUG) JProfiler::getInstance('Application')->mark('beforeJoomActivitiesGetItems');
        ...
        foreach ($items AS &$item)
        {
            ...
            $item = JoomActivitiesHelper::translate($item, $config);
        }

        // Add the items to the internal cache.
        $this->cache[$store] = $items;

        if (JDEBUG) JProfiler::getInstance('Application')->mark('afterJoomActivitiesGetItems');
    ...
    {
        if (JDEBUG) JProfiler::getInstance('Application')->mark('beforeJoomActivitiesGetItemsGrouped');
        ...
        foreach ($items AS &$item)
        {
            ...
            if (count($item) == 1) {
                $key  = key($item);
                $item = JoomActivitiesHelper::translate($item[$key], $config);
            }
            else {
                $item = JoomActivitiesHelper::translate($item, $config);
            }
        ...
        if (JDEBUG) JProfiler::getInstance('Application')->mark('afterJoomActivitiesGetItemsGrouped');
    ...
    public function getEvents()
    {
        ...
        foreach ($items AS &$item)
        {
            ...
            $item->text = JText::_('COM_JOOMACTIVITIES_EVENT_' . strtoupper($item->text));
        }

 ...
 *
 */
class JoomActivitiesModelItem extends JModelAdmin
{
    /**
     ...
     */
    public function &getTable($type = 'Item', $prefix = 'JoomActivitiesTable', $config = array())
    {
        static $table = array();
    ...
    protected function getAssetId($name, $title = null, $create = false)
    {
        ...
        if (!isset($parents[$name])) {
            ...
            $query->clear()
                  ->select('id')
                  ->from('#__assets')
                  ->where('name = ' . $this->_db->quote('com_joomactivities'));
